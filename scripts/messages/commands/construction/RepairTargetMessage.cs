...
public partial class RepairTargetMessage : MessageBase, CommandInterface, QueueableCommandInterface
{
    ...
    public RepairTargetMessage(string p_player_id = "", int p_issued_at_tick = -1, string[] p_repair_unit_ids = default, string p_target_entity_id = "", int p_queue_mode = (int)GameMessages.QueueMode.REPLACE)
    {
    ...
    public override GDictionary to_payload() => D(("player_id", player_id), ("issued_at_tick", issued_at_tick), ("queue_mode", queue_mode), ("repair_unit_ids", repair_unit_ids), ("target_entity_id", target_entity_id));
