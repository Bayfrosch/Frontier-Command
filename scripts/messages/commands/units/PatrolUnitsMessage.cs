...
public partial class PatrolUnitsMessage : MessageBase, CommandInterface, QueueableCommandInterface
{
    ...
    public PatrolUnitsMessage(string p_player_id = "", int p_issued_at_tick = -1, string[] p_unit_ids = default, Vector2 p_destination = default, int p_queue_mode = (int)GameMessages.QueueMode.REPLACE)
    {
    ...
    public override GDictionary to_payload() => D(("player_id", player_id), ("issued_at_tick", issued_at_tick), ("queue_mode", queue_mode), ("unit_ids", unit_ids), ("destination", destination));
